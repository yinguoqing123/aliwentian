...
class NeZhaEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, co_ocurrence_ids=None, inputs_embeds=None, word_embeddings=None):
        """
        :param input_ids: input ids
        :param token_type_ids:  token type ids
        :param co_ocurrence_ids: if co ocurrence ids is None this embedings will cal input_embeds+token_embeds else will cal input_embeds+token_embeds+co_ocurrence_embeds
        :param inputs_embeds:
        :return:
        """
...
class NeZhaLabelHead(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class NeZhaModel(NeZhaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.is_co_ocurrence = config.is_co_ocurrence
        self.init_weights()
    ...
    Examples::
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, token_type_ids=token_type_ids, co_ocurrence_ids=co_ocurrence_ids,
            inputs_embeds=inputs_embeds, word_embeddings=word_embeddings
        )
        # we want to use co ocurrence feature in fineturning stage and we use it to embedding layer
...
class NeZhaForPreTrainingWithLabel(NeZhaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.bert = NeZhaModel(config)
        self.predictions = BertOnlyMLMHead(config)
        # self.cls = BertPreTrainingHeads(config)
        self.label_classifier = NeZhaLabelHead(config)
        self.init_weights()
    ...
    Examples::
        ...
        sop_relationship_score = self.label_classifier(pooled_output)
        # add hidden states and attention if they are here
        outputs = (prediction_scores, sop_relationship_score,) + outputs[2:]

        if labels is not None and matched_labels is not None:
            loss_fct = CrossEntropyLoss()
            masked_lm_loss = loss_fct(prediction_scores.view(-1, self.config.vocab_size), labels.view(-1))
            sop_loss = loss_fct(sop_relationship_score.view(-1, 2), matched_labels.view(-1))
...
class NeZhaForPreTraining(NeZhaPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = NeZhaModel(config)
        self.cls = BertPreTrainingHeads(config)
        # self.sop_cls = NeZhaSOPHead(config)
        self.init_weights()
...
class NeZhaForSequenceClassificationWithHeadClass(NeZhaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = NeZhaModel(config)
...
class NeZhaForSequenceClassificationWithHeadClassMD(NeZhaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.dropout_num = 8
        self.num_labels = config.num_labels
...
class NeZhaForSequenceClassificationWithClsCat(NeZhaPreTrainedModel):
    """
    this class is cat cls and mutil sample dropout
    """

    def __init__(self, config, dropout_num=8):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = NeZhaModel(config)
        # bert pooler is not used and have no grad
        ...
        self.dropouts = nn.ModuleList([nn.Dropout(config.classifier_dropout_prob) for _ in range(dropout_num)])
...
class NeZhaForSequenceClassificationWithClsCatForOnnx(NeZhaPreTrainedModel):
    """
    this class is cat cls and mutil sample dropout
    """

    def __init__(self, config, dropout_num=4):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.bert = NeZhaModel(config)
        # we want use cls in every layers and pooler in here and not in bert
